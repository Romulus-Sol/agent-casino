...
async function main() {
  ...
  try {
    const tx = await program.methods
      .coinFlip(betAmount, choice, Array.from(clientSeed))
      .accounts({
        house: housePda,
        houseVault: vaultPda,
    ...
    console.log("\n--- Result ---");
    console.log("Roll:", result === 0 ? "HEADS" : "TAILS");
    console.log("You chose:", choice === 0 ? "HEADS" : "TAILS");
    ...
    console.error("\nError playing coin flip:", e.message);
    if (e.logs) {
      console.log("\nProgram logs:");
